...
class SpamDbScan extends StatelessWidget {
  SpamDbScan({Key? key}) : super(key: key);
  final controller = TextEditingController();
  @override
  Widget build(BuildContext context) {
    return BlocConsumer<SpamdbCubit, SpamdbState>(listener: (context, state) {
      if (state is SpamdbError) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(state.error!),
          duration: const Duration(seconds: 3),
        ));
      } else if (state is SpamdbLoaded) {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => SpamDbInfo(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          BlocProvider.of<SpamdbCubit>(context)
                              .getSpamDb(controller.text);
                        },
